...
welcomemsg() { \
	dialog --title "Welcome!" --msgbox "Welcome to Siarch Auto-Rice Bootstrapping Script!\\n\\nThis script will automatically install a fully-featured Linux desktop, which I use as my main machine.\\n\\n" 10 60
...
gitmakeinstall() {
	...
	dialog --title "Siarch Installation" --infobox "Installing \`$progname\` ($n of $total) via \`git\` and \`make\`. $(basename "$1") $2" 5 70
...
pipinstall() { \
	dialog --title "Siarch Installation" --infobox "Installing the Python package \`$1\` ($n of $total). $1 $2" 5 70
...
installall() {
	...
	for x in curl base-devel git ntp zsh; do
		dialog --title "Siarch Installation" --infobox "Installing \`$x\` which is required to install and configure other programs." 5 70
		installpkg "$x"
	done

	dialog --title "Siarch Installation" --infobox "Synchronizing system time to ensure successful and secure installation of software..." 4 70
	...
	dialog --title "Siarch Installation" --infobox "Finally, installing \`libxft-bgra\` to enable color emoji in suckless software without crashes." 5 70
...
[ -z "$OUTPUTPATH" ] && OUTPUTPATH="~/.siarch"
